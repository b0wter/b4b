...
tryDeckQueryArgument url =
    let
        ...
        val =
            dict |> Maybe.andThen (Dict.get "deck")
    in
    case val of
        Just (QS.One o) ->
            Just o
...
update msg model =
    case msg of
        ...
        ToggleHelpModal ->
            let
                visibility =
                    if model.helpModalVisibility == Modal.shown then
                        Modal.hidden
        ...
        ConfirmResetModal ->
            let
                updatedModel =
                    { model | selectedCards = [], notSelectedCards = cards, yesNoModalContent = Nothing, yesNoModalVisibility = Modal.hidden }

                url =
                    shareUrl updatedModel |> Url.toString
            in
            ...
            case card of
                Just c ->
                    let
                        ...
                        navUrl =
                            updatedModel |> shareUrl |> Url.toString
                    in
        ...
        ResetCards ->
            let
                updatedModel =
                    { model | selectedCards = [], notSelectedCards = cards }
...
inventoryView model =
    let
        numberOfSelectedCards =
            model.selectedCards |> List.length

        border =
            if numberOfSelectedCards <= maxDeckSize then
                Border.dark
...
inventoryHeaderView numberOfSelectedCards display =
    let
        selectionCountString =
            "(" ++ (numberOfSelectedCards |> String.fromInt) ++ "/" ++ (maxDeckSize |> String.fromInt) ++ ")"
        ...
        yesNoContent =
            { header = "Reset", text = "Clear the currently selected cards?", yesMsg = ConfirmResetModal, noMsg = RejectResetModal }

        styleSelector =
            inventoryStyleToggle display
...
inventoryContentView model =
    let
        ...
        asCardsView cards =
            if cards |> List.isEmpty then
                emptyDeckHint

            else
                cards |> List.indexedMap (\i c -> summaryCardView (Just (i + 1)) c)
    ...
    let
        ...
        numberOfCards =
            case card.supplyLine.name of
                Cards.Nest _ ->
                    Cards.nestSupplyLineCount

                Cards.Alley _ ->
                    Cards.alleySupplyLineCount

                Cards.Clinic _ ->
                    Cards.clinicSupplyLineCount
...
inventorySummaryView cards =
    let
        ...
        hasPassives =
            mergedProperties.passives |> (not << List.isEmpty)

        hasRemaining =
            mergedProperties.remaining |> (not << List.isEmpty)

        hasTeam =
            mergedProperties.team |> (not << List.isEmpty)

        hasDisables =
            mergedProperties.disables |> (not << List.isEmpty)

        orEmptyElement hasElement element =
            if hasElement then
                element
...
inventoryProgressView cards =
    let
        requirements =
            case cards of
                head :: tail ->
                    Cards.supplyLineRequirements ( head, tail )
        ...
        progressBar supplyLineRequirement color label =
            let
                a =
                    supplyLineRequirement.requiredProgress * 100 |> toFloat

                b =
                    supplyLineRequirement.totalElements |> toFloat
