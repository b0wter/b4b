...
parseSupplyLine raw =
    let
        track =
            parseSupplyTrack raw.track raw.name
    in
    { name = track
    , index = raw.index
    , tier = raw.tier
...
parseRawCard raw =
    let
        line =
            parseSupplyLine raw.supplyLine
    in
    { id = raw.id
    , title = raw.name
    , cost = raw.cost
...
containsWord word card =
    let
        ...
        textAndProperties =
            (card.title |> String.toLower) :: descriptions
    in
    textAndProperties
        |> String.join " "
...
groupProperties properties =
    ...
    let
        descriptions =
            properties |> List.map (\p -> p.description)
            ...
            let
                ...
                merger mergeElements =
                    let
                        ...
                        totalValue =
                            mergeElements |> List.foldl folder 0
                    in
                    first
            ...
            grouped
                ...
                |> List.map
                    (\element ->
                        ...
                        in
                        val
                            ++ (if element.absOrRel == Absolute then
                                    " "
...
supplyLineRequirements selection =
    let
        highestIndex predicate ( c, cc ) =
            let
                filtered =
                    (c :: cc) |> List.filter predicate
            in
            filtered
                |> List.maximumWith
                    (\a b ->
                        if a.supplyLine.index > b.supplyLine.index then
                            Basics.GT

                        else if a.supplyLine.index == b.supplyLine.index then
                            Basics.EQ
        ...
        highestNestIndex =
            selection |> highestIndex isNestLine

        highestAlleyIndex =
            selection |> highestIndex isAlleyLine

        highestClinicIndex =
            selection |> highestIndex isClinicLine

        requiredAchievements =
            (selection |> Tuple.first)
                :: (selection |> Tuple.second)
